...
func (s *testParserSuite) TestBRIE(c *C) {
	table := []testCase{
		{"BACKUP DATABASE a TO 'local:///tmp/archive01/'", true, "BACKUP DATABASE `a` TO _UTF8MB4'local:///tmp/archive01/'"},
		{"BACKUP SCHEMA a TO 'local:///tmp/archive01/'", true, "BACKUP DATABASE `a` TO _UTF8MB4'local:///tmp/archive01/'"},
		{"BACKUP DATABASE a,b,c TO 'noop://'", true, "BACKUP DATABASE `a`, `b`, `c` TO _UTF8MB4'noop://'"},
		{"BACKUP DATABASE a.b TO 'noop://'", false, ""},
		{"BACKUP DATABASE * TO 'noop://'", true, "BACKUP DATABASE * TO _UTF8MB4'noop://'"},
		{"BACKUP DATABASE *, a TO 'noop://'", false, ""},
		{"BACKUP DATABASE a, * TO 'noop://'", false, ""},
		{"BACKUP DATABASE TO 'noop://'", false, ""},
		{"BACKUP TABLE a TO 'noop://'", true, "BACKUP TABLE `a` TO _UTF8MB4'noop://'"},
		{"BACKUP TABLE a.b TO 'noop://'", true, "BACKUP TABLE `a`.`b` TO _UTF8MB4'noop://'"},
		{"BACKUP TABLE a.b,c.d,e TO 'noop://'", true, "BACKUP TABLE `a`.`b`, `c`.`d`, `e` TO _UTF8MB4'noop://'"},
		{"BACKUP TABLE a.* TO 'noop://'", false, ""},
		{"BACKUP TABLE * TO 'noop://'", false, ""},
		{"BACKUP TABLE TO 'noop://'", false, ""},
		{"RESTORE DATABASE * FROM 's3://bucket/path/'", true, "RESTORE DATABASE * FROM _UTF8MB4's3://bucket/path/'"},

		{"BACKUP DATABASE * TO 'noop://' LAST_BACKUP = '2020-02-02 14:14:14'", true, "BACKUP DATABASE * TO _UTF8MB4'noop://' LAST_BACKUP = '2020-02-02 14:14:14'"},
		{"BACKUP DATABASE * TO 'noop://' LAST_BACKUP = 1234567890", true, "BACKUP DATABASE * TO _UTF8MB4'noop://' LAST_BACKUP = 1234567890"},

		{"backup database * to 'noop://' rate_limit 500 MB/second snapshot 5 minute ago", true, "BACKUP DATABASE * TO _UTF8MB4'noop://' RATE_LIMIT = 500 MB/SECOND SNAPSHOT = 300000000 MICROSECOND AGO"},
		{"backup database * to 'noop://' snapshot = '2020-03-18 18:13:54'", true, "BACKUP DATABASE * TO _UTF8MB4'noop://' SNAPSHOT = '2020-03-18 18:13:54'"},
		{"backup database * to 'noop://' snapshot = 1234567890", true, "BACKUP DATABASE * TO _UTF8MB4'noop://' SNAPSHOT = 1234567890"},
		{"restore table g from 'noop://' concurrency 40 checksum 0 online 1", true, "RESTORE TABLE `g` FROM _UTF8MB4'noop://' CONCURRENCY = 40 CHECKSUM = 0 ONLINE = 1"},
		{
			...
			true,
			"BACKUP TABLE `x` TO _UTF8MB4's3://bucket/path/?endpoint=https://test-cluster-s3.local&access-key=aaaaaaaaa&secret-access-key=bbbbbbbb&force-path-style=1'",
		},
		{
			...
			true,
			"BACKUP DATABASE * TO _UTF8MB4's3://bucket/path/?provider=alibaba&region=us-west-9&storage-class=glacier&sse=AES256&acl=authenticated-read&use-accelerate-endpoint=1' SEND_CREDENTIALS_TO_TIKV = 1",
		},
		{
			...
			true,
			"RESTORE DATABASE * FROM _UTF8MB4'gcs://bucket/path/?endpoint=https://test-cluster.gcs.local&storage-class=coldline&predefined-acl=OWNER&credentials-file=/data/private/creds.json'",
		},

		{"IMPORT DATABASE * FROM 'file:///data/dump'", true, "IMPORT DATABASE * FROM _UTF8MB4'file:///data/dump'"},
		{
			"import schema * from 'file:///d/' checkpoint false analyze off checksum optional backend 'importer' tikv_importer '10.0.1.1:8287'",
			true,
			"IMPORT DATABASE * FROM _UTF8MB4'file:///d/' CHECKPOINT = 0 ANALYZE = OFF CHECKSUM = OPTIONAL BACKEND = 'importer' TIKV_IMPORTER = '10.0.1.1:8287'",
		},
		{
			"IMPORT DATABASE * FROM 'file:///d/' BACKEND TIDB ON DUPLICATE IGNORE SKIP_SCHEMA_FILES TRUE",
			true,
			"IMPORT DATABASE * FROM _UTF8MB4'file:///d/' BACKEND = 'tidb' ON_DUPLICATE = 'ignore' SKIP_SCHEMA_FILES = 1",
		},
		{
			...
			true,
			"IMPORT DATABASE * FROM _UTF8MB4'file:///d/' CSV_HEADER = COLUMNS STRICT_FORMAT = 1 CSV_BACKSLASH_ESCAPE = 1 CSV_DELIMITER = '''' CSV_NOT_NULL = 0 CSV_NULL = 'Null' CSV_SEPARATOR = '|' CSV_TRIM_LAST_SEPARATORS = 1",
		},
		{"import table db1.tbl1 from 'file:///d/' csv_header = 0", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' CSV_HEADER = 0"},
		{"import table db1.tbl1 from 'file:///d/' csv_header = 1", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' CSV_HEADER = 1"},
		{"import table db1.tbl1 from 'file:///d/' csv_header = 9001", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' CSV_HEADER = 9001"},
		{"import table db1.tbl1 from 'file:///d/' csv_header = fields", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' CSV_HEADER = COLUMNS"},
		{"import table db1.tbl1 from 'file:///d/' csv_header = 'columns'", false, ""},
		{"import table db1.tbl1 from 'file:///d/' on_duplicate = ignore", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' ON_DUPLICATE = 'ignore'"},
		{"import table db1.tbl1 from 'file:///d/' on_duplicate = replace", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' ON_DUPLICATE = 'replace'"},
		{"import table db1.tbl1 from 'file:///d/' on_duplicate = error", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' ON_DUPLICATE = 'error'"},
		{"import table db1.tbl1 from 'file:///d/' backend = local", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' BACKEND = 'local'"},
		{"import table db1.tbl1 from 'file:///d/' backend = tidb", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' BACKEND = 'tidb'"},
		{"import table db1.tbl1 from 'file:///d/' backend = importer", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' BACKEND = 'importer'"},
		{"import table db1.tbl1 from 'file:///d/' checkpoint = 'false'", false, ""},
		{"import table db1.tbl1 from 'file:///d/' checkpoint = 30", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' CHECKPOINT = 1"},
		...
		{"import table db1.tbl1 from 'file:///d/' resume 1", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' RESUME = 1"},
		{"import table db1.tbl1 from 'file:///d/' resume abc", false, ""},
		{"import table db1.tbl1 from 'file:///d/' analyze = optional", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' ANALYZE = OPTIONAL"},
		...
		{"import table db1.tbl1 from 'file:///d/' checksum true analyze 2", true, "IMPORT TABLE `db1`.`tbl1` FROM _UTF8MB4'file:///d/' CHECKSUM = REQUIRED ANALYZE = REQUIRED"},
		...
		{"restore table g from 'noop://' checksum off", true, "RESTORE TABLE `g` FROM _UTF8MB4'noop://' CHECKSUM = 0"},
		{"restore table g from 'noop://' checksum optional", true, "RESTORE TABLE `g` FROM _UTF8MB4'noop://' CHECKSUM = OPTIONAL"},

		{"backup database * to concat('s3://bucket/backup-', now())", true, "BACKUP DATABASE * TO CONCAT(_UTF8MB4's3://bucket/backup-', NOW())"},
	}
...
func (s *testParserSuite) TestEventStatements(c *C) {
	table := []testCase{
		...
		},
		{
			...
			true,
			"CREATE DEFINER = CURRENT_USER EVENT `e` ON SCHEDULE AT NOW() DO BACKUP DATABASE * TO _UTF8MB4'local://backup/'",
		},
		{
